...
_CONFIGS_TABLE_NAME = os.environ.get("CONFIGS_TABLE_NAME", "")
...
def _extract_resource_fields(servico: str, tipo_recurso: str, data: dict) -> dict[str, Any]:
    """Extract the same summary fields that list_resources() returns per service."""
    dados = data.get("dados", {})
    if servico == "MediaLive":
        if tipo_recurso == "channel":
            return {
                "id": data.get("channel_id", ""),
                "nome": data.get("nome_canal", ""),
                "estado": data.get("estado", ""),
                "classe": dados.get("ChannelClass", dados.get("classe", "")),
            }
        if tipo_recurso == "input":
            return {
                "id": data.get("channel_id", ""),
        ...
        if tipo_recurso == "channel_v2":
            return {
                "channel_group": dados.get("ChannelGroupName", dados.get("channel_group", "")),
                "nome": data.get("nome_canal", ""),
                "input_type": dados.get("InputType", dados.get("input_type", "")),
            }
        if tipo_recurso == "origin_endpoint_v2":
            return {
                "channel_group": dados.get("ChannelGroupName", dados.get("channel_group", "")),
                "canal": dados.get("ChannelName", dados.get("canal", "")),
                "endpoint": dados.get("OriginEndpointName", dados.get("endpoint", "")),
                "container": dados.get("ContainerType", dados.get("container", "")),
            }
    elif servico == "MediaTailor":
        if tipo_recurso == "playback_configuration":
            ad_server = dados.get("AdDecisionServerUrl", dados.get("ad_server", ""))
            return {
                "nome": data.get("nome_canal", ""),
                "video_source": dados.get("VideoContentSourceUrl", dados.get("video_source", "")),
                "ad_server": ad_server[:80] + "..." if len(ad_server) > 80 else ad_server,
            }
    elif servico == "CloudFront":
        if tipo_recurso == "distribution":
            comment = dados.get("Comment", dados.get("comment", ""))
            # Try to get a friendly name: Comment > first Alias > domain
            aliases = dados.get("Aliases", {})
            alias_items = aliases.get("Items", []) if isinstance(aliases, dict) else []
            first_alias = alias_items[0] if alias_items else ""
            domain = dados.get("DomainName", dados.get("domain", ""))
            nome = comment or first_alias or domain or data.get("channel_id", "")
            return {
                "id": data.get("channel_id", ""),
                "nome": nome[:80] if nome else "",
                "domain": domain,
                "status": dados.get("Status", dados.get("status", "")),
                "enabled": dados.get("Enabled", dados.get("enabled", False)),
                "comment": comment[:60] if comment else "",
            }
    # Fallback: return basic fields
    return {"id": data.get("channel_id", ""), "nome": data.get("nome_canal", "")}


def _list_resources_dynamodb(
    servico: str, tipo_recurso: str,
) -> dict[str, Any]:
    """Query StreamingConfigs DynamoDB table for resources. Fast alternative to AWS API calls."""
    from boto3.dynamodb.conditions import Key

    table = _dynamodb_resource.Table(_CONFIGS_TABLE_NAME)
    # All normalizers use tipo="configuracao"
    pk_value = f"{servico}#configuracao"
...
def _list_resources_aws(
    servico: str, tipo_recurso: str,
) -> dict[str, Any]:
    """List resources via direct AWS API calls (original implementation)."""
    items: list[dict[str, Any]] = []
        ...
        if tipo_recurso == "distribution":
            ...
            for page in paginator.paginate():
                ...
                for dist in dist_list.get("Items", []):
                    ...
                    nome = comment or first_alias or domain or dist.get("Id", "")
                    items.append({
                        "id": dist.get("Id", ""),
                        "nome": nome[:80] if nome else "",
                        "domain": domain,
...
def list_resources(
    ...
    Tries DynamoDB first (fast) and falls back to AWS API calls if
    the table is not configured or the query fails.
    """
    if _CONFIGS_TABLE_NAME:
        try:
            result = _list_resources_dynamodb(servico, tipo_recurso)
            if result.get("total", 0) > 0:
                logger.info(
                    "list_resources %s/%s via DynamoDB (%d items)",
                    servico, tipo_recurso, result["total"],
                )
                return result
            logger.info(
                "list_resources %s/%s DynamoDB returned 0 items, falling back to AWS API",
                servico, tipo_recurso,
            )
        except Exception as exc:
            logger.warning(
                "list_resources %s/%s DynamoDB failed (%s), falling back to AWS API",
                servico, tipo_recurso, exc,
            )

    logger.info("list_resources %s/%s via AWS API", servico, tipo_recurso)
    return _list_resources_aws(servico, tipo_recurso)
...
def _formatar_duracao(minutos: int) -> str:
    ...
    Regras:
        < 60min   → apenas "Xmin"
        60–1439   → "Xh" ou "XhYmin"
        ≥ 1440    → inclui "Xd", seguido de horas e minutos restantes
    """
    if minutos < 60:
...
def _agrupar_incidentes(
    ...
    Eventos separados por menos de janela_consolidacao_minutos são agrupados
    no mesmo incidente. Retorna lista de incidentes ordenada por inicio ASC.

    Valida: Requisitos 4.1, 4.2, 4.3, 4.4, 4.5, 4.6
    """
    # 1. Filtrar apenas eventos ERROR ou CRITICAL
    eventos_anomalos = [
        e for e in eventos
        if e.get("severidade") in ("ERROR", "CRITICAL")
    ]

    if not eventos_anomalos:
        return []
    ...
    eventos_anomalos.sort(key=_parse_ts)

    # 3. Agrupar eventos consecutivos com gap < janela_consolidacao_minutos
    incidentes: list[dict] = []
    grupo_atual: list[dict] = [eventos_anomalos[0]]

    for evento in eventos_anomalos[1:]:
        ts_atual = _parse_ts(evento)
        ts_ultimo = _parse_ts(grupo_atual[-1])
        gap_minutos = (ts_atual - ts_ultimo).total_seconds() / 60

        if gap_minutos < janela_consolidacao_minutos:
            grupo_atual.append(evento)
        else:
            incidentes.append(grupo_atual)
            grupo_atual = [evento]

    incidentes.append(grupo_atual)

    # 4. Converter cada grupo em Incidente_SLA
    resultado: list[dict] = []
    for grupo in incidentes:
        inicio = _parse_ts(grupo[0])
        fim = _parse_ts(grupo[-1])
        duracao_minutos = int((fim - inicio).total_seconds() / 60) + 5

        # 5. Determinar severidade_maxima usando _SEVERITY_ORDER
        severidade_maxima = max(
            (e.get("severidade", "ERROR") for e in grupo),
            key=lambda s: _SEVERITY_ORDER.get(s, 0),
        )

        # 6. Contar eventos_count como número de metrica_nome distintos
        eventos_count = len({e.get("metrica_nome") for e in grupo if e.get("metrica_nome")})

        resultado.append({
            "inicio": inicio,
            "fim": fim,
            "duracao_minutos": duracao_minutos,
            "duracao_formatada": _formatar_duracao(duracao_minutos),
...
def _consultar_ddb_sla(
    ...
    """Consulta DynamoDB StreamingLogs com PK={servico}#{canal}, SK range.
    ...
    Valida: Requisitos 2.1, 2.2, 2.3, 2.4, 2.5, 13.5
    """
    from boto3.dynamodb.conditions import Key
    ...
    projection_with_alias = "#ts, severidade, tipo_erro, metrica_nome, metrica_valor"

    eventos: list[dict] = []
    last_key = None
    max_tentativas = 3
    backoff_segundos = [1, 2, 4]
        ...
        while tentativa < max_tentativas:
            try:
                kwargs: dict = {
                    ...
                    "ProjectionExpression": projection_with_alias,
                ...
                if code == "ProvisionedThroughputExceededException" and tentativa < max_tentativas - 1:
                    logger.warning(
                        "DynamoDB throttling para PK=%s, tentativa %d/%d. Aguardando %ds.",
                        pk, tentativa + 1, max_tentativas, backoff_segundos[tentativa],
                    )
                    time.sleep(backoff_segundos[tentativa])
                    tentativa += 1
                else:
                    logger.error("Erro ao consultar DynamoDB SLA PK=%s: %s", pk, exc)
                    raise
...
def _consultar_s3_sla_fallback(
    ...
    """Consulta S3 KB_LOGS como fallback quando o DynamoDB falha.

    Lista objetos com prefixo {KB_LOGS_PREFIX}/{servico}/{canal} (ou
    {KB_LOGS_PREFIX}/{canal} se servico=None). Filtra por data de modificação
    dentro do período solicitado. Desserializa cada JSON e extrai os campos
    necessários para o cálculo de SLA.

    Valida: Requisitos 3.1, 3.2, 3.3
    """
    kb_bucket = os.environ.get("KB_LOGS_BUCKET", "")
    kb_prefix = os.environ.get("KB_LOGS_PREFIX", "").rstrip("/")

    if servico:
        prefix = f"{kb_prefix}/{servico}/{canal}" if kb_prefix else f"{servico}/{canal}"
    else:
        prefix = f"{kb_prefix}/{canal}" if kb_prefix else canal

    eventos: list[dict] = []
    continuation_token = None

    while True:
        kwargs: dict = {"Bucket": kb_bucket, "Prefix": prefix}
        if continuation_token:
            kwargs["ContinuationToken"] = continuation_token
        ...
        try:
            ...
            logger.error("Erro ao listar objetos S3 KB_LOGS prefix=%s: %s", prefix, exc)
            raise

        for obj in resp.get("Contents", []):
            last_modified = obj.get("LastModified")
            # Filter by modification date within the requested period
            if last_modified is None:
                continue
            # Ensure last_modified is timezone-aware for comparison
            if last_modified.tzinfo is None:
                last_modified = last_modified.replace(tzinfo=timezone.utc)
            if not (timestamp_inicio <= last_modified <= timestamp_fim):
                continue

            key = obj["Key"]
            try:
                get_resp = s3_client.get_object(Bucket=kb_bucket, Key=key)
                raw = get_resp["Body"].read()
                data = json.loads(raw)
            except (ClientError, json.JSONDecodeError, ValueError) as exc:
                logger.warning("Ignorando arquivo S3 %s — erro ao ler/desserializar: %s", key, exc)
                continue

            # data may be a single event dict or a list of events
            ...
            for record in records:
                ...
                evento = {
                    ...
                    "severidade": record.get("severidade", ""),
                    "canal": record.get("canal", canal),
                    "tipo_erro": record.get("tipo_erro", ""),
                    "metrica_nome": record.get("metrica_nome", ""),
                }
...
def _healthcheck_build_queries(
    ...
    for idx, rec in enumerate(recursos):
        ...
        safe_id = re.sub(r"[^a-zA-Z0-9]", "", res_id)[:30]

        for metric_name, stat in metrics_list:
            safe_metric = metric_name.replace(".", "_").lower()
            if has_pipeline:
                for pipeline in ("0", "1"):
                    qid = f"hc{idx}_{safe_id}_{safe_metric}_p{pipeline}"
                    dims = [
                ...
                qid = f"hc{idx}_{safe_id}_{safe_metric}"
...
def _healthcheck_classify_resources(
    ...
    for rec in recursos:
        ...
        nome = rec.get("nome", "") or rec.get("comment", "") or rec.get("domain", rid)
        name_map[rid] = nome
...
# -------------------------------------------------------------------
# Analytics — aggregated metric queries from DynamoDB
# -------------------------------------------------------------------

_ANALYTICS_METRICS = {
    "MediaPackage": [
        "EgressBytes", "EgressRequestCount",
        "EgressResponseTime", "IngressBytes",
    ],
    "MediaTailor": [
        "Avail.FillRate", "AdDecisionServer.Errors",
        "AdDecisionServer.Ads", "Avail.Duration",
        "Avail.FilledDuration",
    ],
    "MediaLive": [
        "ActiveAlerts", "InputLossSeconds",
        "DroppedFrames", "NetworkIn", "NetworkOut",
    ],
    "CloudFront": [
...
def _clean_channel_name(name: str) -> str:
    """Remove technical suffixes from SpringServe supply tag names.

    Examples:
        "ESPN Argentina - SSLA - Preroll" → "ESPN Argentina"
        "Gran Hermano 24h - Free - CTV - chromecast" → "Gran Hermano 24h"
        "DSports Chile - Web - Preroll" → "DSports Chile"
    """
    import re as _re
    # Remove known device/platform/ad-type suffixes (case-insensitive)
    # Pattern: remove everything from " - " followed by known keywords
    suffixes_pattern = (
        r"\s*[-–]\s*(?:"
        r"Free|CTV|chromecast|Chromecast|Web|App|android[_\s]?mobile|"
        r"android|ios|mobile|tablet|desktop|smart[_\s]?tv|smarttv|"
        r"SSLA|DGO|Sky\+?|production|staging|"
        r"preroll|midroll|postroll|pre-roll|mid-roll|post-roll|"
        r"pre|mid|post|linear|vod|hd|sd|4k|"
        r"BR|AR|CL|CO|PE|MX|EC|UY|PY|BO|VE|"
        r"SSLA|SSLA2|SSLA3"
        r")(?:\s*[-–]\s*.*)?$"
    )
    result = _re.sub(suffixes_pattern, "", name, flags=_re.IGNORECASE).strip()
...
def _execute_analytics(
    ...
    """Query DynamoDB logs for aggregated metrics.

    Returns top N resources ranked by the specified
    aggregation (max, avg, sum, min) of a metric.
    """
    table_name = os.environ.get("LOGS_TABLE_NAME", "") or (
        _CONFIGS_TABLE_NAME.replace("Configs", "Logs")
        if _CONFIGS_TABLE_NAME else ""
    )
    if not table_name:
        raise ValueError(
            "LOGS_TABLE_NAME não configurado"
        )

    # Validate
    valid_metrics = _ANALYTICS_METRICS.get(servico, [])
    if not valid_metrics:
        raise ValueError(
            f"Serviço inválido: {servico}. "
            f"Válidos: {list(_ANALYTICS_METRICS.keys())}"
        )

    if agregacao not in ("max", "avg", "sum", "min"):
        raise ValueError(
            "Agregação inválida. Use: max, avg, sum, min"
        )
    ...
    configs_table_name = _CONFIGS_TABLE_NAME
    now = datetime.now(timezone.utc)
    start = now - timedelta(hours=periodo_horas)
    start_str = start.strftime("%Y-%m-%dT%H:%M:%SZ")

    # Step 1: Get known canals for this servico from configs table
    # (much smaller table, fast Query by GSI)
    canals = []
    if configs_table_name:
        try:
            ctable = _dynamodb_resource.Table(configs_table_name)
            resp = ctable.query(
                IndexName="GSI_NomeCanal",
                KeyConditionExpression=Key("servico").eq(servico),
                ProjectionExpression="nome_canal",
            )
            canals = [
                item["nome_canal"]
                for item in resp.get("Items", [])
                if item.get("nome_canal")
            ]
        except Exception:
            canals = []
    ...
    if canals:
        # Step 2: Query logs table by PK={servico}#{canal} for each canal
        # in parallel — O(canals) queries instead of full table scan
        def _query_canal(canal):
            pk = f"{servico}#{canal}"
            canal_items = []
            q_kwargs = {
                "KeyConditionExpression": (
                    Key("PK").eq(pk)
                    & Key("SK").gte(start_str)
                ),
                "FilterExpression": Attr("metrica_nome").eq(metrica),
                "ProjectionExpression": "canal, metrica_valor",
            }
            while True:
                r = table.query(**q_kwargs)
                canal_items.extend(r.get("Items", []))
        ...
        with ThreadPoolExecutor(max_workers=10) as pool:
            futures = [pool.submit(_query_canal, c) for c in canals]
        ...
        # Fallback: limited scan with page cap to avoid timeout
        scan_kwargs = {
            "FilterExpression": (
                Attr("PK").begins_with(f"{servico}#")
                & Attr("metrica_nome").eq(metrica)
                & Attr("SK").gte(start_str)
            ),
            "ProjectionExpression": "canal, metrica_valor",
            "Limit": 5000,  # cap to avoid timeout
        }
    ...
    if not items:
        return {
            ...
            "ranking": [],
            "mensagem": (
                f"Nenhum dado de {metrica} encontrado "
                f"para {servico} nas últimas "
                f"{periodo_horas}h"
            ),
        }
    ...
    canal_values: dict[str, list[float]] = defaultdict(
        list,
    )
    for item in items:
        canal = item.get("canal", "unknown")
        val = item.get("metrica_valor")
        if val is not None:
            canal_values[canal].append(float(val))

    # Calculate aggregation per canal
    ranking = []
    for canal, values in canal_values.items():
        if not values:
            continue
        if agregacao == "max":
            agg_val = max(values)
        ...
        ranking.append({
            "canal": canal,
            "valor": round(agg_val, 2),
            "amostras": len(values),
        })

    # Sort descending (except min → ascending)
    reverse = agregacao != "min"
    ...
    if "Bytes" in metrica:
        ...
        for r in ranking:
            gb = r["valor"] / (1024 ** 3)
            r["valor_formatado"] = f"{gb:.2f} GB"
    elif "Rate" in metrica:
        unit = "%"
        for r in ranking:
            r["valor_formatado"] = f"{r['valor']:.1f}%"
    elif "Time" in metrica:
        unit = "ms"
        for r in ranking:
            r["valor_formatado"] = f"{r['valor']:.0f}ms"
    else:
        for r in ranking:
            r["valor_formatado"] = f"{int(r['valor']):,}".replace(",", ".")

    # Enrich ranking with friendly channel names from SpringServe correlations
    if configs_table_name and ranking:
        try:
            ctable = _dynamodb_resource.Table(configs_table_name)
            for r in ranking:
                canal = r.get("canal", "")
                if not canal:
                    continue
                try:
                    # Look up in SpringServe correlation table
                    # PK = "Correlacao#canal", SK = mediatailor_name (e.g. "live_1289")
                    resp = ctable.get_item(
                        Key={
                            "PK": "Correlacao#canal",
                            "SK": canal,
                        },
                        ProjectionExpression="supply_tag_name, #d",
                        ExpressionAttributeNames={"#d": "data"},
                    )
                    item = resp.get("Item")
                    if item:
                        nome = item.get("supply_tag_name")
                        if not nome and item.get("data"):
                            import json as _j
                            d = _j.loads(item["data"]) if isinstance(item["data"], str) else item["data"]
                            nome = d.get("supply_tag_name") or d.get("channel_name")
                        if nome and nome != canal:
                            clean = _clean_channel_name(nome)
                            # Add protocol tag based on MT name prefix
                            if canal.startswith("livh_"):
                                clean = f"{clean} [HLS]"
                            r["nome_amigavel"] = clean
                except Exception:
                    pass
        except Exception:
            pass
    ...
    return {
        ...
        "total_canais": len(canal_values),
        "ranking": ranking,
        "metricas_disponiveis": valid_metrics,
        "mensagem": (
            f"Top {len(ranking)} canais {servico} por "
            f"{agregacao}({metrica}) nas últimas "
            f"{periodo_horas}h"
        ),
    }
...
def handler(event: dict, context: Any) -> dict:
    ...
    if api_path == "/consultarMetricas":
        ...
        # Guard: detect SpringServe/ADs queries misrouted here
        _AD_TERMS = {"ads", "ad", "springserve", "spring serve", "anuncio",
                     "anúncio", "supply tag", "demand tag", "fill rate",
                     "impressoes", "impressões", "receita"}
        _rid_lower = resource_id.lower()
        _svc_lower = servico.lower()
        if _rid_lower in _AD_TERMS or any(t in _rid_lower for t in _AD_TERMS) or \
                _svc_lower in ("springserve", "ads"):
            return _bedrock_response(event, 400, {
                "erro": (
                    "Esta operação não suporta métricas de anúncios/SpringServe. "
                    "Para consultar métricas de ADs, use a Knowledge Base KB_ADS "
                    "diretamente (rota ANUNCIOS). Para exportar dados de anúncios, "
                    "use Action_Group_Export com base_dados='KB_ADS'."
                ),
                "redirecionamento": "KB_ADS",
            })
    ...
    if api_path in ("/gerenciarRecurso", "/deletarRecurso", "/startStopCanal", "/listarRecursos"):
        ...
        # --- ANALYTICS ---
        if acao == "analytics":
            metrica = parameters.get("metrica", "")
            if not metrica:
                return _bedrock_response(event, 400, {
                    "erro": (
                        "Parâmetro 'metrica' obrigatório. "
                        "Ex: EgressBytes, Avail.FillRate"
                    ),
                })
            agregacao = parameters.get(
                "agregacao", "max",
            )
            top_n = 10
            raw_top = parameters.get("top_n")
            if raw_top:
                try:
                    top_n = int(raw_top)
                except (ValueError, TypeError):
                    top_n = 10
            periodo_horas = 24
            raw_ph = parameters.get("periodo_horas")
            if raw_ph:
                try:
                    periodo_horas = int(raw_ph)
                except (ValueError, TypeError):
                    periodo_horas = 24
            try:
                result = _execute_analytics(
                    servico=servico or "MediaPackage",
                    metrica=metrica,
                    agregacao=agregacao,
                    top_n=top_n,
                    periodo_horas=periodo_horas,
                )
                # Add chart marker for frontend rendering
                if result.get("ranking"):
                    import json as _json, csv as _csv, io as _io
                    chart_payload = {
                        "labels": [
                            r.get("nome_amigavel") or r["canal"]
                            for r in result["ranking"]
                        ],
                        "values": [r["valor"] for r in result["ranking"]],
                        "formatted": [r.get("valor_formatado", str(r["valor"])) for r in result["ranking"]],
                        "label": f"{agregacao}({metrica})",
                        "unit": result.get("unidade", ""),
                        "title": result.get("mensagem", ""),
                    }
                    result["chart_marker"] = (
                        f"[CHART_DATA:{_json.dumps(chart_payload, ensure_ascii=False)}]"
                    )
                    # Build CSV with full ranking (all canals, not just top_n)
                    csv_buf = _io.StringIO()
                    writer = _csv.writer(csv_buf)
                    writer.writerow(["canal", "nome", "valor", "valor_formatado", "amostras"])
                    for r in result["ranking"]:
                        writer.writerow([
                            r.get("canal", ""),
                            r.get("nome_amigavel") or r.get("canal", ""),
                            r.get("valor", ""),
                            r.get("valor_formatado", ""),
                            r.get("amostras", ""),
                        ])
                    csv_data = csv_buf.getvalue()
                    import base64 as _b64
                    csv_b64 = _b64.b64encode(csv_data.encode("utf-8")).decode("ascii")
                    fname = f"analytics_{servico}_{metrica}_{agregacao}.csv".replace(".", "_")
                    result["csv_marker"] = f"[DOWNLOAD_ANALYTICS_CSV:{fname}:{csv_b64}]"
                ...
                logger.error(
                    "Analytics error: %s", exc,
                )
                return _bedrock_response(event, 500, {
                    "erro": f"Erro no analytics: {exc}",
                })
        ...
        return _bedrock_response(event, 400, {
            "erro": f"Ação inválida: '{acao}'. Use: deletar, start, stop, listar, healthcheck, historico, comparar, analytics",
        })
