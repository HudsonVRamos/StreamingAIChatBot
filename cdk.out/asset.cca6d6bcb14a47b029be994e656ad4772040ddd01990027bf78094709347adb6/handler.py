...
def handler(event: dict, context: Any) -> dict:
    ...
    try:
        ...
        # Return content inline — frontend creates download blob
        # Truncate to 50KB for agent response limit
        inline_content = conteudo[:50000]
        truncated = len(conteudo) > 50000

        return _bedrock_response(event, 200, {
            "mensagem": f"Exportação concluída: {len(dados)} registros em {formato}.",
            "resumo": resumo,
            "dados_exportados": inline_content,
            "formato_arquivo": ext,
            "truncado": truncated,
        })
