...
def _execute_analytics(
    ...
    """
    table_name = _CONFIGS_TABLE_NAME.replace(
        "Configs", "Logs",
    ) if _CONFIGS_TABLE_NAME else os.environ.get(
        "LOGS_TABLE_NAME", "",
    )
    if not table_name:
        raise ValueError(
    ...
    # Scan with filter for servico + metrica + period
    items = []
    scan_kwargs = {
        "FilterExpression": (
            Attr("servico_origem").eq(servico)
            & Attr("metrica_nome").eq(metrica)
            & Attr("SK").gte(start_str)
        ),
        "ProjectionExpression": (
            "canal, metrica_valor, #ts"
        ),
