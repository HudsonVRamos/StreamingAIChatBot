...
def _write_log_to_dynamodb(evento: dict) -> None:
    """Write log event to DynamoDB StreamingLogs.

    Fail-open: errors are logged but do not interrupt
    the S3 storage flow.
    """
    ...
    try:
        ...
        logger.error(
            "DynamoDB write failed (fail-open): %s",
            exc,
        )
