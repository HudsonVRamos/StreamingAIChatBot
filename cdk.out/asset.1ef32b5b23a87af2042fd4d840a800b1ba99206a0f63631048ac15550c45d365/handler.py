...
def query_s3_data(
    ...
    Each object is expected to be a JSON document (Config_Enriquecida
    or Evento_Estruturado).  Objects that fail to parse are skipped.
    """
        ...
        for page in pages:
            for obj in page.get("Contents", []):
                ...
                try:
                    ...
                    logger.warning(
                        "Skipping object s3://%s/%s: %s",
                        bucket, key, exc,
                    )
    except ClientError as exc:
        logger.error(
            "Error listing objects in s3://%s/%s: %s",
            bucket, prefix, exc,
        )
