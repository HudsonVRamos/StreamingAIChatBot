...
def handler(event, context):
    ...
    try:
        with open("DOC_SPRINGSERVER.yml", "r") as f:
            doc_content = f.read()
        s3.put_object(
...
def _process_supply_tags(auth, s3, ddb, results):
    ...
    for raw in raw_tags:
        ...
        config = normalize_supply_tag(raw, priorities)
...
def _extract_supply_tag_id_from_url(url: str) -> str | None:
    ...
    - https://video.springserve.com/vast/{id}?param=val
    ...
    # Check URL path for /vast/{id} pattern
    path = parsed.path.rstrip("/")
    match = re.search(r"/vast/(\d+)", path)
    if match:
...
def _process_correlations(
    ...
    supply tags via ad_decision_server_url parsing.

    Validates: Requirements 6.1, 6.2, 6.3, 6.4, 6.5, 6.6
    """
    logger.info("Processando correlações MediaTailor ↔ SpringServe")

    # Build lookup: supply_tag_id (str) → normalized supply tag
    stag_by_id = {}
    ...
    for cfg in mt_configs:
        ...
        if tag_id and tag_id in stag_by_id:
            ...
            # Find report data for this supply tag
            report_data = None
            # (report data would come from results
            #  collected earlier; for now pass None)
            corr = normalize_correlation(
                cfg, matched_tag, report_data
            )
